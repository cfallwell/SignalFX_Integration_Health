...
variable "auth_token" {
  type        = string
  sensitive   = true
  description = "Splunk Observability Cloud API token with detector-creation permissions"
}
...
variable "enable_synthetics" {
  type        = bool
  default     = true
  description = "Enable the Synthetics API test for custom metrics"
}
...
1. Filter the 'AWS namespace coverage' list chart in this org by namespace = {{dimensions.namespace}}. The chart is created by terraform/charts.tf — see the namespace_coverage_chart_url output.
