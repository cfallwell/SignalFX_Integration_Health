locals {
  # Message templates for each rule - extracted from detector_messages/*.md files
  rule_messages = {
...
EOT
  ...
  # Severity defaults - can be overridden by var.rule_severity_overrides
  default_severities = {
  ...
  rule_severities = merge(
    local.default_severities,
    var.rule_severity_overrides
  )
}
