...
1. Open the "AWS namespace coverage" list chart created by the Terraform module (see `namespace_coverage_chart_url` output). Filter by namespace = {{dimensions.namespace}} to see every AWS integration that covers this namespace — those are the candidates.  
